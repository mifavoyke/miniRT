...
/*   Updated: 2025/05/15 23:25:27 by zuzanapiaro      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minirt.h"

static void	init_inputs(t_inter *inters, t_light_math *vars, t_coord lightpoint, t_coord viewpoint)
{
	t_coord	offset;

	vars->normal = get_surface_normal(inters);
	normalize(&vars->normal);
	vars->shadow_ray = make_vector(inters->point, lightpoint);
	...
	vars->shadow_origin = move_point_by_vector(inters->point, offset);
	vars->incident_l = make_vector(inters->point, lightpoint);
	normalize(&vars->incident_l);
	vars->scalar_nl = dot(vars->incident_l, vars->normal);
	vars->incident_v = make_vector(viewpoint, inters->point);
	normalize(&vars->incident_v);
	vars->reflectivity = 0.0;
}

// final colour = original colour*(1 - reflectivity)+light colour*reflectivity
static t_colour	apply_light(t_colour orig, t_colour light, float reflect)
{
	t_colour	color;

	color.r = fmax(0, fmin(255, orig.r * (1 - reflect) + light.r * reflect));
	color.g = fmax(0, fmin(255, orig.g * (1 - reflect) + light.g * reflect));
	color.b = fmax(0, fmin(255, orig.b * (1 - reflect) + light.b * reflect));
	color.a = 255;
	return (color);
}
...
static t_colour	compute_light_contribution(t_scene *scene, t_inter *inter, t_light *l, t_coord viewpoint)
{
	...
	if (is_in_shadow(scene, &light_inputs, inter->id) || inter->id == -42
		|| light_inputs.scalar_nl <= EPS || (dot(light_inputs.normal,
				light_inputs.incident_v) >= 0.5))
