...
/*   Updated: 2025/05/09 16:45:01 by zpiarova         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_inter *get_coat_inters(t_coord origin, t_coord base_ray_vector, t_coord ray_axis_cross, float root_part, t_cylinder *cy, t_coord ray)
{
	float	t1;
	float	t2;
	float	d1;
	...
	{
		inter1 = make_inter((void *)cy, d1, ray, origin);
	...
	{
		inter2 = make_inter((void *)cy, d2, ray, origin);
...
t_inter	*get_cap_inters(t_coord ray, t_coord origin, t_coord base_ray_vector, t_cylinder *cy)
{
	float	top_t;
	float	bottom_t;
	float	top_d;
	...
	if (get_dot_product(cy->vector, ray) != 0)
	{
		...
		{
			inter3 = make_inter((void *)cy, top_d, ray, origin);
		...
		{
			inter4 = make_inter((void *)cy, bottom_d, ray, origin);
